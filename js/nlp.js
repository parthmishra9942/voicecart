 ...
 * decision is explainable, has zero running cost, and is privacy-first.
 *
...
function normalizeLangText(text, lang) {
  ...
  if (
    L === 'hi' ||
    /\b(doodh|dudh|ande|anda|chawal|aloo|aaloo|pyaz|pyaaz|tamatar|karo|kar do|hatao|dikhao|dhoondo|dhundo)\b/i.test(t)
  ) {
    t = t
      .toLowerCase()
      .replace(/[०-९]/g, c => String('०१२३४५६७८९'.indexOf(c)));

    for (const [a, b] of Object.entries(HINGLISH)) {
      t = t.replace(
        new RegExp('\\b' + a.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b', 'gi'),
        b
      );
    }

    t = t
      .replace(/\bkar\s+do\b/g, '')
      .replace(/\bkar\s+dena\b/g, '')
      .replace(/\badd\s+do\b/g, 'add')
      .replace(/\bhatao\s+do\b/g, 'remove');
  }
...
/* -- trim leading conversational words (please/okay) ----------------------- */
function leadTrim(tokens) {
  ...
  while (i < tokens.length && leads.has(tokens[i])) {
    i++;
  }
...
/* -- greedy pattern match, returns captured %IT% string --------------------- */
function matchPattern(pattern, seq) {
  ...
  if (tail.length) {
    for (let t = 0; t < tail.length; t++) {
      if (seq[seq.length - tail.length + t] !== tail[t]) {
        return null;
      }
    }
  }
  ...
  const cap = seq.slice(
    i,
    tail.length ? seq.length - tail.length : seq.length
  );
...
function resolveProduct(str) {
  ...
  if (ALIAS_MAP[s]) {
    return idToProduct(ALIAS_MAP[s]);
  }
  ...
  for (const w of words) {
    if (ALIAS_MAP[w]) {
      const p = idToProduct(ALIAS_MAP[w]);
  ...
  if (sing && ALIAS_MAP[sing]) {
    const p = idToProduct(ALIAS_MAP[sing]);
  ...
  /*
   * Closest-match against a single product name.
   * Multi-word phrases without an alias are not good fuzzy candidates.
   */
  const sWords = normalize(str)
    .split(' ')
    .filter(Boolean);
...
function extractQtyAndUnit(captured) {
  const tokens = normalize(captured)
    .split(' ')
    .filter(Boolean);
...
function parse(transcript, lang) {
  ...
  const text = normalize(
    normalizeLangText(transcript, L)
  );

  const tokens = leadTrim(
  ...
  const priceMatch = text.match(
    /(?:under|below|less than)\s*(?:\$|usd\s*)?(\d+(?:\.\d+)?)/
  );

  if (priceMatch) {
    priceUnder = parseFloat(priceMatch[1]);
  }

  /*
   * Intent detection.
   * Specific/destructive intents get priority over additive behavior.
   */
  const order = [
    'remove',
    'list',
  ...
  for (const intentKey of order) {
    const patterns =
      (INTENT_PATTERNS[intentKey] &&
       INTENT_PATTERNS[intentKey][L]) || [];
    ...
    for (const pat of sorted) {
      ...
      if (cap !== null) {
        ...
        /*
         * Price bounds always imply search.
         */
        if (priceUnder) {
          intent = 'search';
        }
   ...
   * If nothing matched, treat the complete input as the item.
   */
  if (
    intent === 'add' &&
    captured === '' &&
  ...
  if (
    ...
    /*
     * For price searches, don't interpret the price as a quantity.
     */
    if (priceUnder && priceMatch) {
      captureForEntity = captureForEntity
        .replace(priceMatch[0], ' ')
        .replace(
          /\b(dollars|dollar|usd|bucks)\b/g,
          ' '
        );
    }
    ...
    const {
      ...
      unit,
      name
    } = extractQtyAndUnit(captureForEntity);
